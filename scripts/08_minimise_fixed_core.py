...
source=ROOT/f'template55_source{s}_UNRELAXED.pdb';fixed=ROOT/f'template55_source{s}_prepared.pdb'
...
for a in atoms:
 ...
 canmove=(isadapter and (pos>=28 or a.name not in backbone)) or (not isadapter and r.index in flexres and a.name not in backbone)
...
start=time.time();mm.LocalEnergyMinimizer.minimize(ctx,10,200)
...
out=ROOT/f'template55_source{s}_LOCAL_MINIMIZED.pdb'
...
report={'sample':s,'openmm_version':mm.__version__,'status':'bounded_local_minimization_completed_not_binding_validated','forcefield':['amber14-all.xml','implicit/gbn2.xml'],'nonbonded_cutoff_nm':2,'hydrogen_pH':7,'added_terminal_oxygens':added,'seed':5500+s,'fixed':'all penton atoms outside 8A adapter shell; penton backbone inside shell; adapter backbone residues1-27','movable_atoms':len(movable),'adapter_CA28_52_position_restraint_k_kJ_mol_nm2':1000,'distance_contact_restraints':False,'max_iterations':200,'tolerance_kJ_mol_nm':10,'initial_energy_kJ_mol':e0,'final_energy_kJ_mol':e1,'movable_force_rms_kJ_mol_nm':float(np.sqrt(np.mean(np.sum(forces[movable]**2,axis=1)))),'convergence_note':'iteration limit bounded; do not assume force convergence merely from return','elapsed_seconds':time.time()-start,'output':str(out)}
(ROOT/f'relaxation_{s}.json').write_text(json.dumps(report,indent=2));print(json.dumps(report),flush=True)
